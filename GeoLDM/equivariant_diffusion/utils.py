...
def sample_center_gravity_zero_gaussian_with_mask(size, device, node_mask, StP_dataset="none"):
    assert len(size) == 3
    x = torch.randn(size, device=device)
    # node mask -> bsize, #max_nodes, 1
    if StP_dataset == "none":
      x_masked = x * node_mask
    else: 
      norm_value_list = norm_value_list_qm9 if StP_dataset == "QM9" else norm_value_list_geom
