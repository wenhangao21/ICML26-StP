...
data_file = '../processed_dataset/geomgeom_drugs_30.npy'
...
# print(model)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, args.n_epochs, eta_min=args.lr_min)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        if epoch % args.test_epochs == 0:
            dist_loss_val, node_loss_val = train_test.test(args, dataloaders['val'], epoch, model_ema_dp, device, dtype,
                                      property_norms, nodes_dist, partition='Val')
            dist_loss_test, node_loss_test = train_test.test(args, dataloaders['test'], epoch, model_ema_dp, device, dtype,
