...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def phi(self, x, t, node_mask, edge_mask, context):
        edge_mask = edge_mask + torch.eye(node_mask.size(1), device=edge_mask.device).unsqueeze(0)
...
class NonEquiAE(torch.nn.Module):

    def __init__(
            self,
            encoder: torch.nn.Module,
            decoder: torch.nn.Module,
            rot_encoder: torch.nn.Module,
            rot_head: torch.nn.Module,
            num_classes: int,
            include_charges: bool):
        super().__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.rot_encoder = rot_encoder
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes, node_mask):
        ...
        z_x = utils.sample_center_gravity_zero_gaussian_with_mask(
            ...
            node_mask=node_mask)
        z_h = utils.sample_gaussian_with_mask(
            size=(n_samples, n_nodes, self.encoder.out_node_nf), device=node_mask.device,
            node_mask=node_mask)
        z = torch.cat([z_x, z_h], dim=2)
        return z
    
    def rotate(self, x, h, node_mask=None, edge_mask=None):
        xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
        z_rot = self.rot_encoder._forward(xh, node_mask=node_mask, edge_mask=edge_mask, context=None)
        z_rot = z_rot.sum(dim=1) / node_mask.sum(dim=1)
        z_rot = self.rot_head(z_rot).reshape(-1, 3, 3)
        rot_mat = roma.special_procrustes(z_rot)
        x_rot = torch.bmm(x, rot_mat)
        return x_rot
    
    def encode(self, x, h, node_mask=None, edge_mask=None):
        xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
        #diffusion_utils.assert_correctly_masked(xh, node_mask)
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.encoder._forward(xh, node_mask=node_mask, edge_mask=edge_mask, context=None)
        return z_x_mu, -1, z_h_mu, -1

    def decode(self, z_xh, node_mask=None, edge_mask=None):
        pred_x, pred_h = self.decoder._forward(z_xh, node_mask=node_mask, edge_mask=edge_mask, context=None)
        pred_cat = pred_h[..., :self.num_classes]
        pred_int = pred_h[..., self.num_classes:]
        return pred_x, pred_cat, pred_int
    
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        x = self.rotate(x, h, node_mask=node_mask, edge_mask=edge_mask)
        z_x_mu, _, z_h_mu, __ = self.encode(x, h, node_mask=node_mask, edge_mask=edge_mask)
        z_xh_mean = torch.cat([z_x_mu, z_h_mu], dim=2)
        
        # Following GeoLDM: https://github.com/MinkaiXu/GeoLDM/blob/main/equivariant_diffusion/en_diffusion.py (lines 1143-1153),
        # we use a fixed variance for the encoder.
        z_xh = z_xh_mean + 0.00316 * self.sample_combined_position_feature_noise(z_xh_mean.size(0), z_xh_mean.size(1), node_mask=node_mask)
        pred_x, pred_cat, pred_int = self.decode(z_xh, node_mask=node_mask, edge_mask=edge_mask)
        
        x_diff = torch.sum((pred_x - x) ** 2, dim=2) * node_mask.squeeze()
        x_loss = torch.mean(x_diff)
        
        ## reconstruct node types
        node_loss = F.cross_entropy(pred_cat.reshape(-1, self.num_classes), h['categorical'].reshape(-1, self.num_classes).argmax(dim=1))

        if self.include_charges:
            node_loss = node_loss + torch.mean(((pred_int - h['integer']) ** 2) * node_mask)

        return x_loss, node_loss
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def log_constants_p_h_given_z0(self, h, node_mask):
        ...
        n_nodes = node_mask.squeeze(2).sum(1)  # N has shape [B]
        assert n_nodes.size() == (batch_size,)
        degrees_of_freedom_h = n_nodes * self.in_node_nf
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        Computes the loss (type l2 or NLL) if training. And if eval then always computes NLL.
        """
        x = self.vae.rotate(x, h, node_mask=node_mask, edge_mask=edge_mask)
        z_x_mu, _, z_h_mu, __ = self.vae.encode(x, h, node_mask, edge_mask)
        z_xh_mean = torch.cat([z_x_mu, z_h_mu], dim=2)

        # Following GeoLDM: https://github.com/MinkaiXu/GeoLDM/blob/main/equivariant_diffusion/en_diffusion.py (lines 1143-1153),
        # we use a fixed variance for the encoder.
        z_xh = z_xh_mean + 0.00316 * self.vae.sample_combined_position_feature_noise(z_xh_mean.size(0), z_xh_mean.size(1), node_mask=node_mask)
        ...
        neg_log_pxh = loss_ld + neg_log_constants
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        ...
        diffusion_utils.assert_correctly_masked(z_xh, node_mask)
        pred_x, pred_cat, pred_int = self.vae.decode(z_xh, node_mask=node_mask, edge_mask=edge_mask)

        return pred_x, {'categorical': pred_cat, 'integer': pred_int}
