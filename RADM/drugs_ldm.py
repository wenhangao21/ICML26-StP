...
kwargs = {'entity': args.wandb_usr, 'name': args.exp_name, 'project': 'drugs', 'config': args,
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        ldm_train_test.train_epoch(args, dataloaders['train'], epoch, model, model_dp, model_ema, ema, device, dtype,
                               property_norms, optim, nodes_dist, gradnorm_queue, dataset_info,
                               prop_dist)
        print(f"Epoch took {time.time() - start_epoch:.1f} seconds.")

        if epoch > 5 and epoch % args.test_epochs == 0:
            nll_val = ldm_train_test.test(args, dataloaders['val'], epoch, model_ema_dp, device, dtype,
            ...
            if nll_val < best_nll_val:
                ...
                if args.save_model:
                    ...
                    if args.save_model and epoch > 50: # only save current best for large epochs, RADM weights are too large
            ...
            if args.save_model and epoch > 50 and epoch % 10 == 0:
                args.current_epoch = epoch + 1
                epoch_dir = f"outputs/{args.exp_name}/epoch_{epoch}"
                os.makedirs(epoch_dir, exist_ok=True)
                utils.save_model(optim, f"outputs/{args.exp_name}/epoch_{epoch}/optim.npy")
